...
describe('Conciliation', () => {
    describe('Builder', () => {
        it('should build a Conciliation with all required fields', () => {
            const now = new Date();
            const conciliation = Conciliation.Builder
            ...
            expect(conciliation.id).toBe('123e4567-e89b-12d3-a456-426614174000');
            expect(conciliation.runAt).toBe(now);
            expect(conciliation.status).toBe('completed');
            expect(conciliation.summary).toEqual({ matched: 5, discrepancies: 2, missing: 1 });
        });
        ...
        it('should use default summary with zeros when not set', () => {
            ...
            expect(conciliation.summary).toEqual({ matched: 0, discrepancies: 0, missing: 0 });
        });
